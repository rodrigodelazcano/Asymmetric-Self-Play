...
@attr.s(auto_attribs=True)
class BaseEnvConstants(RobotEnvConstants):
...
@attr.s(auto_attribs=True)
class BaseEnvParameters(RobotEnvParameters):
    robot_control_params: BaseRobotControlParameters = build_nested_attr(
...
PType = TypeVar("PType", bound=BaseEnvParameters)
CType = TypeVar("CType", bound=BaseEnvConstants)
SType = TypeVar("SType", bound=AliceSimulationInterface)


class BaseEnv(RobotEnv[PType, CType, SType], abc.ABC):
    """
    ...
    def build_simulation(cls, constants: CType, parameters: PType) -> SType:
        ...
        simulation_type = get_generic_param_type(cls, 2, AliceSimulationInterface)
