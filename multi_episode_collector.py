...
# class MultiEpisodeCollector(SampleCollector):
...
#         # Sorted by env_id (key values)
#         self.policy_collector_groups: Dict[EnvID, List[_PolicyCollectorGroup]] = {}
...
#         # Flag to build batch from collected episodes in an environment
#         # self.build_next_batch: List[EnvID] = []
...
#         # if values["infos"].get("build_next_batch"):
#             # self.build_next_batch.append(env_id) 
#         if not agent_done:
#             self._add_to_next_inference_call(agent_key)
...
#             if pre_batch[SampleBatch.INFOS][-1]["build_next_batch"]:
#                 build_next_batch = True
